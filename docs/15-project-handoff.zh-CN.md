...
> **2026-09-13 追加：设计师真实约束与协商闭环。** 设计师可新增/修改/撤销约束（类别、作用对象或关联偏好、内容、理由、限制性质、提出者、确认状态），仅记录实际输入；成员可读。`important`/`critical` 且关联到已确认偏好的约束会按规则派生 `preference_vs_constraint` 冲突；属性/约束/冲突变化会清除过时审批并回到 `alignment`；未解决冲突不会被自动标记为已解决。真实流程不再注入固定预算约束（`build_fixture_agents` 仅供隔离测试）。接口：`POST/PATCH /v1/projects/{id}/constraints`、`POST /constraints/{id}/withdraw`、`GET /constraints`。
...
- 设计师约束轮验证（在图片轮之后）：后端 206 项、前端 52 项测试通过，Ruff/构建通过；1 条双账户浏览器流程覆盖“设计师录入约束 → 派生冲突 → 屋主回应 → 重新生成说明书 → 双方审批”。
- **仍未实现**：真实视觉/语言模型推理、自然语言约束抽取、云部署、装修效果图生成。真实上传不等于真实图像理解；当前每张图仍产生四条固定模拟观察，必须由屋主确认，不能当作模型识别结果。
