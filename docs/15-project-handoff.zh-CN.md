...
- 设计师约束轮验证（在图片轮之后，含两轮验收修复）：后端 213 项、前端 55 项测试通过，Ruff/构建通过；1 条双账户浏览器流程覆盖“设计师录入约束 → 派生冲突 → 屋主回应 → 生成说明书 → 双方审批 → 审批后修改约束 → 方案过时 → 重新生成 → 双方重新审批”。方案过时时的普通编辑会先按当前状态重建，`realign` 保留用户已编辑的 `goals`，冲突身份绑定约束版本与匹配取值。
