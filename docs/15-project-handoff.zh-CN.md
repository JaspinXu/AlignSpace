...
- 试运行保障验证（本轮）：后端 237 项、前端 58 项测试通过，Ruff 与构建通过；浏览器套件 5 条连续 3 次通过（`auth-workflow` 1 + `collaboration-resilience` 4）。内部模拟试用与备份恢复证据见 `docs/reports/trial-readiness-validation.zh-CN.md`。
