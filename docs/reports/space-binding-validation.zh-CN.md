...
## 11. 第四轮评审：三方同步模型（2026-09-21 追加）

评审指出继续只补删除分支不够，改为统一的**三方合并**（原始基线 / 编辑器草稿 / 服务器当前）：

- `upstreamDiff()` 只写「草稿相对基线真正改动过」的字段；服务器侧的改动若草稿未触碰，保持不变，不再被旧快照回退。
- 双方修改同一字段且取值不同 → 明确报告冲突，保留协作者版本，不写入。
- 删除仅针对「基线中存在且草稿已移除」的实体，且若协作者已修改该实体则报告冲突而不删除；协作者新增实体不在基线中，天然不受影响。
- 编辑器新建实体在创建响应返回后记录为会话内基线（`createdBase`），后续编辑以它为基线做三方比较。

组件层同步状态修正：

- 待同步草稿与其**原始基线绑定保存**（`pendingSyncRef = { project, basePlan }`），轮询刷新/重新导入不得覆盖该基线。
- 存在未解决草稿或冲突时，2D→3D 的重新导入被暂停，避免丢弃草稿。
- 冲突信息独立展示（`role="alert"`），"重试同步"只刷新服务器目标与版本，仍以草稿原始基线做三方合并，不隐含授权删除他人新增内容。

新增测试：`spaceAdapter.test` 三方用例（不回退协作者改动、同字段冲突、房间改名不回退）；`SpaceBoard.test` 组件级「刷新 + 409 + 重试」链路，断言重试不产生 DELETE、不回退 `obj-b`、协作者新增 `obj-collab` 未被触碰。

实测：后端 336、前端 **164**、浏览器 **15**、Ruff/构建/`tsc` 通过。

> **真实 DeepSeek 联调待用户配置密钥后验收。**

## 12. 第五轮评审：同对象不同字段独立合并（2026-09-21 追加）

评审复现：基线 `x=1000,width=800`；设计师只改 `width=1200`；屋主只改 `x=1500`；合并结果却是 `x=1500,width=800`，设计师的宽度被回退且无冲突提示。

根因：位置与尺寸虽分别做了冲突判断，但任一组允许写入时提交整份 `mine.geometry`，把另一组旧值写回。

修复：以**服务器当前 `geometry`** 为基底，只覆盖用户实际改动且通过冲突检查的字段组（位置 `x/y`、尺寸 `width/depth/height` 独立处理）。一组冲突不影响另一组合法改动。新增三项回归：用户移动+协作者改尺寸、用户改尺寸+协作者移动、仅位置冲突时仍应用尺寸。评审提供的只读复现脚本现输出 `x=1500, width=1200`。

更正：上一轮我错误按 `d39ea04` 复核并据此回应，特此更正；本轮评审的 HEAD `ce190b2` 确为当时最新，其两项修复有效。本轮在此基础上修复上述字段级覆盖。

实测：后端 336、前端 **167**（空间相关 48）、浏览器 **15**、Ruff/构建/`tsc` 通过。
