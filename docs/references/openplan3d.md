...
- 编辑回传：前端 `SpaceBoard` 收到 `alignspace:project` 后先用 `upstreamDiff()` 与权威计划做差，把房间名称/几何（矩形或多边形轮廓）、家具位置、编辑器新增的房间/家具、以及删除（删除仅屋主）经**现有受控 API** 落库。安全规则：
  - **非法快照整体拒绝**：任一墙体端点或家具坐标缺失/非有限即忽略整份快照并提示，绝不解译为删除。
  - **三方合并**：只写「草稿相对基线真正改动」的字段；服务器侧（协作者）改动若草稿未触碰则保持不变，不再被旧快照回退。位置与尺寸**按字段组独立合并**（以服务器当前几何为基底），一组冲突不阻断另一组；双方修改同一字段且不同则报告冲突并保留协作者版本。
  - **删除只针对导入基线**：只删除 `basePlan` 中存在且编辑器已移除的 ID（协作者已修改则报告冲突），协作者新增内容不会被旧草稿重试删除。
  - **草稿与基线绑定**：待同步草稿保存其原始基线，轮询刷新或重新导入不会替换该基线；存在冲突时暂停重新导入以保留草稿。
  - **临时 ID 映射**：编辑器新建实体的临时 ID 在创建响应返回后映射为后端 ID，后续编辑走 `update_*`，不会重复创建并删除原件。
  - 同步以**导入时的基准版本**逐次推进；并发冲突返回 409 时保留草稿并显示“重试同步”，不会用最新版本覆盖他人改动。
  - 不支持的形状会明确提示而不是静默矩形化。
