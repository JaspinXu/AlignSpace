...
- **普通编辑（`edit_brief`）无论方案是否过时，都以后端权威状态重建系统字段**（项目、状态、完整度、
  版本、审批、属性、约束、冲突、未决项），仅叠加白名单字段（当前仅 `goals`，需为字符串列表）；
  客户端提交的属性/约束/冲突/完整度等系统字段一律被忽略，无法通过说明书编辑接口覆盖。
  仅当客户端提供的 `project.id` 与路由项目不一致时返回 400。
