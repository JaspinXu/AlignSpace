...
- **来源图片删除**：引用该资产的 `proposed` 观察被移除，**同时从待答问题中移除该图片的选项**；若某个待答问题因此没有选项，则标记为 `skipped`（不再作为 pending），并**推进工作流**生成下一条有效问题。提交已失效选项返回 400；“删除 + 推进”作为**同一个幂等操作**：先写“处理中”记录（`completed=false`），推进成功后再标记“已完成”并缓存最终响应；同键重试若发现“处理中”会**补做未完成的推进**，若“已完成”则直接返回缓存响应，因此推进失败或最终响应保存失败都可恢复，且不会重复推进。已确认偏好保留并显示“来源图片已删除”。不得因此卡住工作流。
