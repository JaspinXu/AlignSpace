...
> 状态：**草案，待用户确认角色权限后定稿**（见第 7 节的两个待确认问题）。
...
**待确认问题：**
1. **空间草稿的编辑权**：设计师是否与屋主**同等**编辑同一份空间草稿（共享可写），还是设计师只能「提议」（提议需屋主确认后才成为空间版本）？
2. **删除权**：删除房间/对象是否也应允许设计师执行，还是仅限屋主（删除会移动对方已建立的绑定）？

若用户未答复，默认采用：**设计师可编辑草稿但不拥有删除权；绑定与「确认应用」仅屋主**。
