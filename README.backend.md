...
- 屋主结构化访谈：先对参考图片中的**部位**做广问，再按所选部位的维度细化；结构化回答（喜欢 / 不在意 / 跳过 + 备注）写入共享偏好，并与图片、部位和来源 evidence 关联；最多 10 个屋主问题；不为未提及属性批量建空记录。
- 每条偏好可追溯到具体参考图片（`evidence.sourceId`）、部位（`targetElement`）与维度（`dimension`）；删除来源图片会移除其**未确认观察**，已确认偏好保留并显示来源已删除。
