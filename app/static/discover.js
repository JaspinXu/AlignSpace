...
(() => {
  ...
  function imageMarkup(home, loading = 'lazy') {
    return `<div class="home-photo"><span class="photo-fallback" aria-hidden="true"><span translate="no">${esc(home.title)}</span><small>View photography at the source ↗</small></span><img src="${esc(home.imageUrl)}" alt="${esc(home.title)} by ${esc(home.designer)}, via Qanvast" loading="${loading}" referrerpolicy="no-referrer"></div>`;
  }
  ...
  function renderHomes() {
    ...
    el('home-grid').innerHTML = visible.map(home => `<article class="home-card">
      ...
      <div class="home-card-body"><p class="home-style">${esc(home.style)}</p><h3><button type="button" data-detail-home="${esc(home.id)}"><span translate="no">${esc(home.title)}</span> <span>↗</span></button></h3><p class="home-cue">${esc(home.styleCue)}</p><div class="preview-tags" aria-label="Source image tags">${home.features.slice(0,3).map(tag => `<span>${esc(tag)}</span>`).join('')}</div><div class="home-card-bottom"><span>${esc(home.propertyType)} · ${esc(area(home))}</span><label class="compare-choice"><input type="checkbox" data-compare-home="${esc(home.id)}" ${compared.includes(home.id) ? 'checked' : ''}> Compare</label></div><a class="photo-credit" translate="no" href="${esc(home.sourceUrl)}" target="_blank" rel="noopener noreferrer">${esc(home.designer)} · Qanvast ↗</a></div>
